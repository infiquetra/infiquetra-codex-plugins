...
CAMPPS history may include retired Mount Olympus values such as `Assigned`, `In Review`, or
`Deployed`; tooling may include those for historical calculations but should not create new
movement with those names.
...
For CAMPPS, active work is `In Progress`. For intent-flow boards, active work is
`Active` plus `Verify`. `Committed`, `Ready`, and `Shaping` are wait or preparation
states unless a card's evidence shows otherwise.
