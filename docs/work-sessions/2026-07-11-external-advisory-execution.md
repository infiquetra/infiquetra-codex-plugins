...
## Final Verified Workflow closure

- The authoritative recovery run is `record:workflow-run:fb81e254b3139e9d05b3497040723e9095ace8ec6dfc12d792db8b7c894719b1` over protected subject `record:subject:b1b23dc620c3538257619a43db244552de8d2f59f35af80c721048bc59b12c88`.
- The current workspace could not be the authoritative gate root because the unrelated, user-owned tracked `.serena/project.yml` modification made its Git baseline unsafe. The final workflow ran in a disposable local clone of commit `1918f3a9b18953ccafbcc0ab6c24fa03037859fe`; the user file was not touched, stashed, staged, or copied into the subject.
- The subject contains the external-advisory implementation, latest proof, and retained evidence bundle. It excludes `scripts/validate_codex_plugins.py` and the two `docs/validation/verified-workflows-*` artifacts because the gate correctly forbids Verified Workflows from approving its own implementation or proof surfaces. Those paths remain covered by the separate repository cutover validator.
- U1-U8 are root revalidation receipts over the committed subject and do not claim that the original implementation was workflow-launched.
