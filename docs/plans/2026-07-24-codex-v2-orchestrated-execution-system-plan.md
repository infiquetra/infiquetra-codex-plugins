...
**Goal:** Support bounded external assistance without granting external results gate or workspace authority.
...
**Approach:** Import approved external rows from the workflow contract into the existing action lifecycle and approval fingerprint. Keep current CLI routes advisory and read-only, pass a minimal environment, materialize only declared secret-screened context, and fail closed on every non-empty external write set until an enforceable filesystem boundary exists. Bind HTTP execution to the exact canonical registry invocation and derive its egress host from the executable URL rather than caller-supplied route metadata. Return route status and artifact identity to the concise run record. Reject gate-shaped provider fields. Allow the root to adopt an independently verified external issue as an ordinary root-owned typed finding.
...
**Test scenarios:** Advisory CLI execution receives only read tools and an allowlisted environment. A non-empty write set, inherited secret variable, undeclared context, secret-bearing path or content, undecodable context, substituted HTTP endpoint or authentication environment, widened executable egress, or external Git mutation fails closed. Success, failure, timeout, invalid output, and operator rejection remain visible and non-gating. A root-adopted finding must be independently verified before entering the native gate.
