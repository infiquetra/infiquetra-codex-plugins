...
- R26. A tool-capable external engine may read or transmit only its declared context, may write only explicitly assigned non-overlapping paths in the shared workspace after approval, and must never receive literal credentials or secret-bearing paths regardless of approval. Response-only providers must remain artifact-only.
- R27. External workers must follow the same write-ownership audit and failure behavior as native workers and must never perform Git operations.
...
- AE4. **Concurrent writes stay disjoint.** **Given:** a native worker and an external engine are approved to edit the shared workspace concurrently. **When:** their assignments are launched. **Then:** each has a distinct write set, neither performs Git operations, and the root reviews both contributions before integration. **Covers R17, R18, R26, R27.**
