...
profile install or plugin cutover. Start a fresh Codex CLI task after the copies exist and inspect the
host's child state plus first turn context. Record `attested` only when the child has the mapped
`agent_role`, model, effort, and sandbox. If the active collaboration tool still omits `agent_type`
or equivalent, retain `inline-only` and record the negative receipt; reloading cannot manufacture a
selector absent from the tool schema.
...
- U1-U4 execute with Saga `orchestration_mode=inline`; U4A-U4F have separate focused evidence and commits. The run pauses before U5, and U5-U8 begin only from the reviewed Workflow Structure plus an `attested` named-profile capability.
