...
|---|---|---|---|---|---|
| Repo marketplace entries | `.agents/plugins/marketplace.json` | `blueprint-reviewer`, `sdlc-manager` | Yes | `replace-in-u8` | Replace with `saga`, `deploy`, `mission-control`, and `team-execution` entries during U8. |
| Repo validator expectations | `scripts/validate_codex_plugins.py`, `tests/test_validate_codex_plugins.py` | `blueprint-reviewer`, `sdlc-manager`, `blueprint-review`, `issue-review`, `spec-review`, `sdlc-board`, `sdlc-flow`, `sdlc-issues`, `sdlc-labels`, `sdlc-metrics`, `sdlc-milestones`, `sdlc-rollout` | Yes | `update-before-u8` | Add staged `current`, `target-fixture`, and `cutover` validation in U2, then flip final active inventory in U8. |
| Repo pytest discovery | `pyproject.toml` | `plugins/sdlc-manager/tests` | Yes | `update-before-u8` | Add new plugin test paths as they land; remove old path during U8. |
| Repo active inventory docs | `README.md`, `docs/baseline/codex-visible-plugins.md`, `docs/validation.md`, `docs/cutover/cache-replacement.md` | old active plugin and skill identifiers | Yes | `replace-in-u8` | Update to the eight-plugin Saga-family target and exact migration map. |
| Current old plugin source roots | `plugins/sdlc-manager/`, `plugins/blueprint-reviewer/` | all old active plugin and skill identifiers | Yes | `lineage-only` until U8, then delete | Source remains only until proof gates pass. Final active aliases are not allowed. |
...
## Required U8 Migration Rows

The cutover docs must include reachable migration rows for at least these old
operator-facing invocations:

| Old invocation | Required replacement |
|---|---|
| `sdlc-manager:sdlc-board` / `sdlc-board` / `/sdlc-board` | `mission-control:board` |
| `sdlc-manager:sdlc-flow` / `sdlc-flow` | `mission-control:flow` |
| `sdlc-manager:sdlc-issues` / `sdlc-issues` / `/create-issue` / `/sdlc-create` / `/sdlc-triage` | `mission-control:issues`, with `mission-control:flow` for project-field helpers |
| `sdlc-manager:sdlc-labels` / `sdlc-labels` | `mission-control:labels` |
| `sdlc-manager:sdlc-metrics` / `sdlc-metrics` / `/sdlc-metrics` | `mission-control:metrics` |
| `sdlc-manager:sdlc-milestones` / `sdlc-milestones` | `mission-control:milestones` |
| `sdlc-manager:sdlc-rollout` / `sdlc-rollout` | `mission-control:rollout` |
| `blueprint-reviewer:blueprint-review` / `blueprint-review` / `/blueprint-review` | `saga:doc-review` |
| `blueprint-reviewer:spec-review` / `spec-review` / `/spec-review` | `saga:spec` or `saga:doc-review` routing to `saga:spec` |
| `blueprint-reviewer:issue-review` / `issue-review` / `/issue-review` | `saga:doc-review`; `mission-control:issues` for GitHub comment mutation |

## Deletion Gate

Deletion is blocked until validator coverage proves this inventory has a
disposition for every confirmed active hit class and the U8 cutover docs expose
the required migration rows.
