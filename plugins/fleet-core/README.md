...
model-catalog, profile-resolution, bridge-proof, lease, concurrency, orphan-evidence, compatibility,
and stateless rate-limit primitives. It exposes no skills, commands, or agents.
...
| `scripts/fleet_commons/retry_backoff.py` | Stateless bounded 429 retry helper retained for UniFi consumers. |
| `scripts/fleet_commons/bridge_receipt.py` / `output_attestation.py` | External-engine execution and output schemas. |
| `scripts/fleet_commons/lease_broker.py` / `concurrency_policy.py` / `orphan_evidence.py` | Cross-runtime lease, admission, and orphan evidence contracts. |
...
Verified Workflows owns the seven current V2 profiles and their role mapping. Fleet Core's older
...
Fleet Core deliberately does not own agent lifecycle, delegation state, audit stores, prompt effort
riders, cost weights, circuit breakers, or documentation renderers. Codex 0.146 and the calling
plugins own those concerns.
