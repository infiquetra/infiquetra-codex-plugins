...
Execution is split at the Verified Workflows bootstrap boundary. U1-U4 ran inline under the root Codex thread as six sequential, independently reviewable subunits. The run paused, source inspection identified the Sol/Terra MultiAgent V2 namespace/bootstrap contract, and saved-config fresh tasks then proved named-profile selection with differential child role, model, and effort readback. Current V2 inherits the parent permission profile after role selection, so permission-specific attempts use fresh permission-homogeneous parents and verify effective child context. U5-U8 may now resume from the durable `## Workflow Structure`; the full Verified Workflows hook/profile/result join remains U8 cutover evidence and never becomes its own acceptance authority.
...
- Codex 0.144.1 reports models, multi-agent, hooks, goals, and plugins as live surfaces, but Saga still mixes Goal, hooks, subagents, fork, and Workflow into one backend vocabulary. The original task exposed the restricted Sol/Terra V2 schema; source inspection and fresh runtime proof established that `hide_spawn_agent_metadata=false` plus `tool_namespace="agents"`, `agent_type`, and a non-full-history fork enable named-profile dispatch.
...
KTD14. Treat generic-child role, model, effort, and read-only labels as requested scope rather than enforced isolation until the active runtime proves otherwise. Run root-owned product-source implementation in `workspace-write` when the host supports it, dispatch named children through the permission-homogeneous parent boundary in KTD22, record every effective permission mode, and reserve U8's real-profile mutation for a separate root-only boundary after isolated proof; if the host forces broader permissions, requested-read-only child evidence remains advisory and no child receives an external-mutation task.
...
The canonical runbook version is allowed to advance when a later unit materially changes the normative port procedure. U4 advanced it from v1 to v2 for host-attested selection, fresh-attempt remediation, and prompt-free evidence rules; the V2 namespace/fork correction advances it to v3. Each checkpoint updates the contract's supported runbook version, active manifest authority digest, generated classification, legacy-token inventory, and version assertions atomically.
...
KTD20. Distinguish named-child selection from Verified Workflows receipt attestation: selection is the host/runtime capability to request one of U3's five profiles and verify the resulting child turn context; receipt attestation is the U4/U8 hook and result chain that binds the selected profile to one workflow role and structured result. Root-verified native `agent_role`, model, effort, and effective permission readback is sufficient to establish the runtime capability needed to start U5-U8. The full hook/profile/result join remains required wherever Verified Workflows claims its own gate-authoritative receipt, and it never accepts changes to its own implementation.

KTD21. Bootstrap Sol/Terra MultiAgent V2 explicitly: model-selected V2 defaults hide `agent_type`, model, effort, and service-tier inputs under the reserved `collaboration` namespace. The effective user and trusted-project Codex config must set `hide_spawn_agent_metadata=false` and `tool_namespace="agents"`; a fresh task then dispatches named profiles with `agent_type=<runtime_agent_name>` and `fork_turns=none` by default. Omitted or `all` is a full-history fork that inherits parent agent type/model/effort and rejects overrides. A differential fresh-task proof must show the planned child role, model, and effort before U5 starts. Missing schema fields or mismatched readback restores the U4 pause instead of falling back to a generic child.

KTD22. Enforce child permissions through permission-homogeneous parents. Installed-tag and current-main V2 apply the named role, then reapply the live parent permission profile, so a profile's `sandbox_mode` cannot narrow a more-powerful parent. Read-only scanner, reviewer, and monitor attempts run under fresh read-only parent tasks; `test_medium` runs under workspace-write. Host-issued rollout `turn_context` proves effective permission, model, effort, and role; child prose proves none of them. The five TOMLs retain their sandbox declarations as policy intent and future-compatible configuration, but current gate logic compares the observed parent-inherited permission with that intent and blocks a mismatch.
...
**Goal:** Add `## Workflow Structure`, root-owned step dispatch and follow-up, logical-role receipts, truthful inline/deterministic fallback, minimal SubagentStart/SubagentStop hook evidence, and a machine-checkable gate with explicit hard-failure precedence. Complete U4 in six sequential inline subunits, including the U4F runtime-identity correction, then produce the U5-U8 workflow handoff and pause.

**Requirements:** R7, R8, R9, R12, R14, R20, R21, R22.

**Dependencies:** U3.

**Files:** `.codex/config.toml`, `.codex/agents/*.toml`, `plugins/verified-workflows/agents/*.toml`, `plugins/verified-workflows/hooks/hooks.json`, `plugins/verified-workflows/hooks/agent_receipt.py`, `plugins/verified-workflows/scripts/render_codex_agents.py`, `plugins/verified-workflows/scripts/sync_codex_agents.py`, `plugins/verified-workflows/scripts/workflow_dispatch.py`, `plugins/verified-workflows/scripts/dispatch_receipt.py`, `plugins/verified-workflows/scripts/protected_store.py`, `plugins/verified-workflows/scripts/workspace_evidence.py`, `plugins/verified-workflows/scripts/workflow_records.py`, `plugins/verified-workflows/scripts/named_child_attestation.py`, `plugins/verified-workflows/scripts/gate_evaluator.py`, `plugins/verified-workflows/scripts/protocol_probe.py`, `scripts/prove_verified_workflows_runtime.py`, `scripts/port_contract.py`, `scripts/build_legacy_workflow_inventory.py`, `scripts/validate_codex_plugins.py`, `docs/portability/claude-to-codex-plugin-port-runbook.md`, `docs/portability/ports/2026-07-10-saga-07517.json`, `docs/portability/codex-saga-07517-drift-classification.md`, `docs/validation/verified-workflows-runtime-proof.json`, `docs/validation/verified-workflows-legacy-token-inventory.json`, `plugins/verified-workflows/skills/run/SKILL.md`, `plugins/verified-workflows/skills/run/references/workflow-protocol.md`, `plugins/verified-workflows/skills/run/references/gate-policy.md`, `plugins/verified-workflows/skills/run/references/validator-evidence-state.md`, `plugins/verified-workflows/skills/run/references/worker-manifest.md`, `plugins/verified-workflows/skills/run/references/delegation-safety.md`, `plugins/verified-workflows/tests/test_workflow_dispatch.py`, `plugins/verified-workflows/tests/test_protected_store.py`, `plugins/verified-workflows/tests/test_workspace_evidence.py`, `plugins/verified-workflows/tests/test_workflow_records.py`, `plugins/verified-workflows/tests/test_named_child_attestation.py`, `plugins/verified-workflows/tests/test_dispatch_receipt.py`, `plugins/verified-workflows/tests/test_gate_evaluator.py`, `plugins/verified-workflows/tests/test_protocol_probe.py`, `tests/test_prove_verified_workflows_runtime.py`, `tests/test_verified_workflows_orchestration_regressions.py`, `tests/test_port_runbook.py`, `tests/test_port_contract.py`, `tests/test_validate_codex_plugins.py`, `tests/test_verified_workflows_migration.py`.
...
Keep the named-child machinery because it is what makes precise model/effort delegation provable. Separate three claims: selection proves the host accepted one of U3's five named profiles; native child-context proof verifies the resulting role, model, effort, and effective parent-inherited permission needed to release the U5 start gate; Verified Workflows receipt attestation additionally joins hook-observed model, installed-profile digest, expected effort bound by that digest, role/lens digest, permission mode, child/task identity, and result reference. The hook accepts at most 64 KiB, records only allowlisted identifiers/model/profile/permission/timestamps, computes the managed profile digest itself, and writes atomic raw events beneath contained `PLUGIN_DATA` directories.
...
The tracked non-live proof records `diagnostic`, `inline-only`, or `auth-unavailable`; a separate root-owned fresh-task record may establish runtime selection `attested` without embedding a transcript. Runtime selection `attested` enables the model-pinned U5-U8 workflow, while a `verified-workflow-subagent` gate claim still requires the complete hook/profile/result chain. U4 retains raw capture, safe pair loading, normalized persistence, consumption-marker recovery, and readback; abandonment, prune, and deletion move to U8 because they are installed hook-data lifecycle work, not model-selection proof.

U4E also lands the runbook v3 runtime-bootstrap correction as one contract transaction. Update `port_contract.py` and its tests to support the declared active version, refresh the manifest's plan/review/runbook authority entries from exact bytes, render the classification, and regenerate the legacy-token inventory after the plan, decision, and review artifacts are final. A manually edited digest or a runbook version rollback is not acceptable.

#### U4F. Codex Runtime Identity and Project Discovery Correction

Keep the five stable execution-class IDs in kebab case, but map each to a separate Codex runtime
agent name using only lowercase letters, digits, and underscores: `review_high`, `review_max`,
`test_medium`, `scan_low`, and `monitor_low`. Generated TOML `name` fields, hook matchers, launch
records, workflow rows, normalized receipts, and runtime proof facts use the runtime agent identity;
policy lookup, plan vocabulary, and fleet-core continue to use the execution class. Never use a task
name or agent path as a substitute for either identity.

Publish exact regular-file project discovery copies under `.codex/agents/` and validate them
byte-for-byte against the plugin source. This is development activation only, not the U8 global
profile install or plugin cutover. Bootstrap V2 per KTD21, start a fresh Codex CLI task, and inspect
the host's child state plus first turn context. Record runtime selection `attested` only when the
spawn call carries the mapped `agent_type` and a non-full-history fork and the child has the mapped
`agent_role`, model, effort, and sandbox. Preserve the earlier negative task-name-only probe as a
regression fixture: it proves that hidden metadata and task naming do not select a profile.
...
**Test scenarios:** Happy path: U4A emits ready intents; U4B preserves all existing public schemas and CLI behavior across extracted modules; U4C produces replay-safe inline/deterministic/result/root records; U4D blocks unresolved severity or validator failures; U4E yields `verified-workflow-subagent` only for a matching explicit selector plus role/class/profile/lens/model/digest/result chain; U4F keeps execution class and runtime agent identity distinct, proves project discovery bytes, and records the actual fresh-child role/model/effort/sandbox result; and runbook v3 plus its manifest/classification/inventory artifacts validate from exact bytes. Edge cases: duplicate or out-of-order events, incomplete pairs retained for later maintenance, backpressure, max-thread exhaustion, stale profile or lens digest, allowed risk escalation, deterministic tool step, root follow-up to a running or idle child, unavailable named selector, unavailable isolated authentication, called-twice intent/normalization/gate evaluation, and a historical manifest retaining runbook v1 remain deterministic and idempotent. Error paths: a helper claiming it spawned a child, task naming or prompt steering reported as selection, automatic credential copy/symlink, project discovery symlink or byte drift, secret-bearing proof, dependency cycle, unsatisfied barrier, required-independence role forced inline, peer messaging required by the plan, oversized/malformed event, prompt-bearing field, payload-supplied path/digest, path escape, unsafe permissions, unknown/generic profile, missing trust, mismatched model, missing result, forged child, unresolved P0/P1/security finding, required validator failure, numeric score used to override severity, active runbook version/schema mismatch, stale authority digest, or stale legacy-token inventory blocks or escalates.
...
**Verification:** Each U4A-U4F checkpoint passes focused tests and is committed separately; the extracted facade remains compatible; project discovery accounts for exactly five byte-current runtime agents; a crash after prepared-marker persistence recovers only from exact normalized readback; no live report claims `verified-workflow-subagent` without explicit selection plus a complete receipt chain; no helper claims native spawn authority; the sanitized tracked proof names its capability outcome and hook/profile hashes; separate fresh-task receipts prove `agent_type`, non-full-history fork, child role, model, and effort, while a permission-homogeneous parent plus child context proves the effective boundary; gate output is deterministic pass/block/escalate with contained evidence; runbook v3, contract validation, generated classification, and legacy inventory are byte-current; the integrated U4 suite and U4 manifest gate pass. The root writes `## Workflow Structure` into this plan at `docs/plans/2026-07-10-codex-plugin-model-execution-modernization-plan.md#workflow-structure` and records that pointer in the Saga handoff. Matching differential role/model/effort and effective-permission receipts release the U4 pause for U5; `diagnostic`, `inline-only`, `auth-unavailable`, or mismatched readback restores the pause rather than substituting a generic child.
...
- Bootstrap phase: U1-U4 use Saga `inline`; U4A-U4F execute sequentially in the root thread with no child implementation or acceptance dependency.
...
The root model and effort are selected explicitly before execution; `xhigh` is sufficient for the bounded root implementation and integration work. The configured V2 spawn surface exposes `agent_type`, model, effort, and service-tier inputs through the non-reserved `agents` namespace. Profile-selected work uses `fork_turns=none` by default. The fresh Sol/xhigh-to-`scan_low` proof recorded `agent_role=scan_low`, Luna/low, and read-only, so the runtime selection gate is satisfied for U5. The complete hook/profile/result receipt remains a separate Verified Workflows and U8 cutover gate.
...
focused test + root diff audit + atomic commit
              ...
              explicit operator pause
                         |
       selector + child-context capability gate (passed)
                         |
...
|---|---|---|---|---|
| U5 | All Saga state/schema/ledger source changes and migration decisions | Base `architecture-reviewer` and `devils-advocate-reviewer` at `review-max`; base `security-reviewer` at `review-high`; `concurrency-tester` at `test-medium` | Launch-state, append-only migration, crash/replay, idempotency, and regression evidence | U4F `attested`; Saga persists inline until U5 canonical fields land |
...
At the pause, regenerate the table's effective model/effort and profile digests from the current fleet-core catalog rather than copying stale values. The Workflow Structure records the exact logical role ID, role/lens digest, execution class, runtime agent name, resolved model/effort/profile digest, independence, files, mutation boundary, dependencies, expected evidence, retry policy, and stop conditions. If U4F is not `attested`, retain this table as the ready handoff and stop.
...
| Role profiles and workflow receipts | U3-U4 | Root-only inline bootstrap; U4A-U4F are sequential checkpoints and Verified Workflows remains the system under test. |
...
- U4A-U4F each receive a root diff audit plus focused checks, then one integrated U4 regression pass; no child or Verified Workflows gate accepts the bootstrap. The formal `/code-review` later supplies the independent work-to-PR review.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| u5-implement | - | - | root | root | n/a | - | - | root | root-only | u5-implement-evidence | - | - | - | - | n/a | n/a | - |
| u5-architecture-review | u5-implement | - | architecture-reviewer | agent-lens | preferred | review-max | review_max | subagent | none | review-evidence | e48b37cea0b26bf39cae4d6611b4219e907d52d284ba6b9489b523a4b16c835f | 3a6cf0a6e31eba23e2f00084e914c2a00b929079f96e47a4b14707b073bd1ad7 | gpt-5.6-sol | max | n/a | n/a | - |
| u5-devils-review | u5-implement | - | devils-advocate-reviewer | agent-lens | preferred | review-max | review_max | subagent | none | review-evidence | 129f6dca0702ffcd4be7f9e5d0939e8e6806788846ba4058044c931883ef0e63 | 3a6cf0a6e31eba23e2f00084e914c2a00b929079f96e47a4b14707b073bd1ad7 | gpt-5.6-sol | max | n/a | n/a | - |
| u5-security-review | u5-implement | - | security-reviewer | agent-lens | preferred | review-high | review_high | subagent | none | review-evidence | bf5bc1b66c0ee3d06071976b659c522c23057c56de5f6cc010556b2653c86980 | 4385da916a043cbd2d0cc227b2f079af7d11fdf0c4eff57daed6dc9a96a1bc85 | gpt-5.6-sol | high | n/a | n/a | - |
| u5-concurrency-test | u5-implement | - | concurrency-tester | agent-lens | preferred | test-medium | test_medium | subagent | none | tester-evidence | d40188645b7876e32ea592dd9799ee2ad7a2e230d82341611708dd492837b3da | fca08003678c3aa14af45ab2f4c63d2ae63d331e2f6a5caae89b62d962f57229 | gpt-5.6-terra | medium | true | false | - |
| u5-integrate | u5-architecture-review,u5-devils-review,u5-security-review,u5-concurrency-test | - | root | root | n/a | - | - | root | root-only | u5-integrate-evidence | - | - | - | - | n/a | n/a | - |
| u6-implement | u5-integrate | - | root | root | n/a | - | - | root | root-only | u6-implement-evidence | - | - | - | - | n/a | n/a | - |
| u6-architecture-review | u6-implement | - | architecture-reviewer | agent-lens | preferred | review-high | review_high | subagent | none | review-evidence | e48b37cea0b26bf39cae4d6611b4219e907d52d284ba6b9489b523a4b16c835f | 4385da916a043cbd2d0cc227b2f079af7d11fdf0c4eff57daed6dc9a96a1bc85 | gpt-5.6-sol | high | n/a | n/a | - |
| u6-devils-review | u6-implement | - | devils-advocate-reviewer | agent-lens | preferred | review-high | review_high | subagent | none | review-evidence | 129f6dca0702ffcd4be7f9e5d0939e8e6806788846ba4058044c931883ef0e63 | 4385da916a043cbd2d0cc227b2f079af7d11fdf0c4eff57daed6dc9a96a1bc85 | gpt-5.6-sol | high | n/a | n/a | - |
| u6-security-review | u6-implement | - | security-reviewer | agent-lens | preferred | review-high | review_high | subagent | none | review-evidence | bf5bc1b66c0ee3d06071976b659c522c23057c56de5f6cc010556b2653c86980 | 4385da916a043cbd2d0cc227b2f079af7d11fdf0c4eff57daed6dc9a96a1bc85 | gpt-5.6-sol | high | n/a | n/a | - |
| u6-testing-review | u6-implement | - | testing-reviewer | agent-lens | preferred | review-high | review_high | subagent | none | review-evidence | a867575e24c86b0573485d1d8bbd81514af3654d544342677b85f4bed0d9af63 | 4385da916a043cbd2d0cc227b2f079af7d11fdf0c4eff57daed6dc9a96a1bc85 | gpt-5.6-sol | high | n/a | n/a | - |
| u6-scenario-test | u6-implement | - | scenario-tester | agent-lens | preferred | test-medium | test_medium | subagent | none | tester-evidence | 8167b31e38f328eca0bf4cfc4ad782ee3a85669af7b08be8aa422b8edbc46f68 | fca08003678c3aa14af45ab2f4c63d2ae63d331e2f6a5caae89b62d962f57229 | gpt-5.6-terra | medium | true | false | - |
| u6-concurrency-test | u6-implement | - | concurrency-tester | agent-lens | preferred | test-medium | test_medium | subagent | none | tester-evidence | d40188645b7876e32ea592dd9799ee2ad7a2e230d82341611708dd492837b3da | fca08003678c3aa14af45ab2f4c63d2ae63d331e2f6a5caae89b62d962f57229 | gpt-5.6-terra | medium | true | false | - |
| u6-integrate | u6-architecture-review,u6-devils-review,u6-security-review,u6-testing-review,u6-scenario-test,u6-concurrency-test | - | root | root | n/a | - | - | root | root-only | u6-integrate-evidence | - | - | - | - | n/a | n/a | - |
| u7-implement | u6-integrate | - | root | root | n/a | - | - | root | root-only | u7-implement-evidence | - | - | - | - | n/a | n/a | - |
| u7-architecture-review | u7-implement | - | architecture-reviewer | agent-lens | preferred | review-high | review_high | subagent | none | review-evidence | e48b37cea0b26bf39cae4d6611b4219e907d52d284ba6b9489b523a4b16c835f | 4385da916a043cbd2d0cc227b2f079af7d11fdf0c4eff57daed6dc9a96a1bc85 | gpt-5.6-sol | high | n/a | n/a | - |
| u7-devils-review | u7-implement | - | devils-advocate-reviewer | agent-lens | preferred | review-max | review_max | subagent | none | review-evidence | 129f6dca0702ffcd4be7f9e5d0939e8e6806788846ba4058044c931883ef0e63 | 3a6cf0a6e31eba23e2f00084e914c2a00b929079f96e47a4b14707b073bd1ad7 | gpt-5.6-sol | max | n/a | n/a | - |
| u7-security-review | u7-implement | - | security-reviewer | agent-lens | preferred | review-max | review_max | subagent | none | review-evidence | bf5bc1b66c0ee3d06071976b659c522c23057c56de5f6cc010556b2653c86980 | 3a6cf0a6e31eba23e2f00084e914c2a00b929079f96e47a4b14707b073bd1ad7 | gpt-5.6-sol | max | n/a | n/a | - |
| u7-security-scan | u7-implement | - | security-scanner | agent-lens | preferred | scan-low | scan_low | subagent | none | scanner-evidence | 54c03db73fa75650e95c93ea642a38fdcc342f57315c3b8bf2d96b2431e63cb9 | be7d14b175ebf1d09785542e6acd50349bdb699fed8d767409814dc780567856 | gpt-5.6-luna | low | true | false | - |
| u7-scenario-test | u7-implement | - | scenario-tester | agent-lens | preferred | test-medium | test_medium | subagent | none | tester-evidence | 8167b31e38f328eca0bf4cfc4ad782ee3a85669af7b08be8aa422b8edbc46f68 | fca08003678c3aa14af45ab2f4c63d2ae63d331e2f6a5caae89b62d962f57229 | gpt-5.6-terra | medium | true | false | - |
| u7-integrate | u7-architecture-review,u7-devils-review,u7-security-review,u7-security-scan,u7-scenario-test | - | root | root | n/a | - | - | root | root-only | u7-integrate-evidence | - | - | - | - | n/a | n/a | - |
| u8-implement | u7-integrate | - | root | root | n/a | - | - | root | root-only | u8-implement-evidence | - | - | - | - | n/a | n/a | - |
| u8-architecture-review | u8-implement | - | architecture-reviewer | agent-lens | preferred | review-high | review_high | subagent | none | review-evidence | e48b37cea0b26bf39cae4d6611b4219e907d52d284ba6b9489b523a4b16c835f | 4385da916a043cbd2d0cc227b2f079af7d11fdf0c4eff57daed6dc9a96a1bc85 | gpt-5.6-sol | high | n/a | n/a | - |
| u8-devils-review | u8-implement | - | devils-advocate-reviewer | agent-lens | preferred | review-high | review_high | subagent | none | review-evidence | 129f6dca0702ffcd4be7f9e5d0939e8e6806788846ba4058044c931883ef0e63 | 4385da916a043cbd2d0cc227b2f079af7d11fdf0c4eff57daed6dc9a96a1bc85 | gpt-5.6-sol | high | n/a | n/a | - |
| u8-security-review | u8-implement | - | security-reviewer | agent-lens | preferred | review-max | review_max | subagent | none | review-evidence | bf5bc1b66c0ee3d06071976b659c522c23057c56de5f6cc010556b2653c86980 | 3a6cf0a6e31eba23e2f00084e914c2a00b929079f96e47a4b14707b073bd1ad7 | gpt-5.6-sol | max | n/a | n/a | - |
| u8-smoke-test | u8-implement | - | smoke-tester | agent-lens | preferred | test-medium | test_medium | subagent | none | tester-evidence | 30004d950c02721615f9abf944207abb3f288d275d1297802e91d97ce73a476a | fca08003678c3aa14af45ab2f4c63d2ae63d331e2f6a5caae89b62d962f57229 | gpt-5.6-terra | medium | true | false | - |
| u8-github-monitor | u8-smoke-test | - | github-actions-monitor | agent-lens | preferred | monitor-low | monitor_low | subagent | none | monitor-evidence | 20739ac1a57c49234d78e2021602f5b3b8e5df3b152bde0ff3471e6fbaf7cd17 | 63ad313aca645525ea09841172a3bea4a65907d20bb389a634aac4570cdace28 | gpt-5.6-luna | low | true | false | - |
| u8-integrate | u8-architecture-review,u8-devils-review,u8-security-review,u8-smoke-test,u8-github-monitor | - | root | root | n/a | - | - | root | root-only | u8-integrate-evidence | - | - | - | - | n/a | n/a | - |
...
| The effective V2 config or active child-spawn surface loses named-profile controls or readback | A requested child may run with a host-selected model and inherited permissions. | Require the non-reserved `agents` namespace, visible metadata, `agent_type`, a non-full-history fork, and differential child receipt before every fresh workflow session; restore the U4 pause on drift. |
...
- U1-U4 execute with Saga `orchestration_mode=inline`; U4A-U4F have separate focused evidence and commits. The reviewed Workflow Structure plus a fresh differential `agent_type`/child-context receipt releases U5, while any later schema or readback drift restores the pause.
...
- Project and later installed readback prove all five profile policies; runtime selection proof additionally binds `agent_type`, non-full-history fork, child role, model, effort, and sandbox. Verified Workflows receipt claims still require matching role/class/lens/profile/result evidence; no generic child is counted as Verified Workflows.
...
Review this material amendment with `saga:doc-review`. After approval, resume `saga:work` at U4A in the current root thread using `gpt-5.6-sol` / `xhigh`, complete U4A-U4F sequentially with focused checks and atomic commits, write the U5-U8 Workflow Structure, refresh named-profile capability proof, and pause. Do not start U5 until the operator confirms the handoff and U4F reports `attested`; Verified Workflows never accepts its own output.
