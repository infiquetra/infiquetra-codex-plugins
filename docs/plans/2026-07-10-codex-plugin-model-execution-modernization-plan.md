...
Execution is split at the Verified Workflows bootstrap boundary. U1-U4 ran inline under the root Codex thread as six sequential, independently reviewable subunits. The run paused, source inspection identified the Sol/Terra MultiAgent V2 namespace/bootstrap contract, and a saved-config fresh task then proved named-profile selection with differential child model, effort, role, and sandbox readback. U5-U8 may now resume from the durable `## Workflow Structure`; the full Verified Workflows hook/profile/result join remains U8 cutover evidence and never becomes its own acceptance authority.
...
**Verification:** Each U4A-U4F checkpoint passes focused tests and is committed separately; the extracted facade remains compatible; project discovery accounts for exactly five byte-current runtime agents; a crash after prepared-marker persistence recovers only from exact normalized readback; no live report claims `verified-workflow-subagent` without explicit selection plus a complete receipt chain; no helper claims native spawn authority; the sanitized tracked proof names its capability outcome and hook/profile hashes; a separate fresh-task receipt proves `agent_type`, non-full-history fork, child role, model, effort, and sandbox; gate output is deterministic pass/block/escalate with contained evidence; runbook v3, contract validation, generated classification, and legacy inventory are byte-current; the integrated U4 suite and U4 manifest gate pass. The root writes `## Workflow Structure` into this plan at `docs/plans/2026-07-10-codex-plugin-model-execution-modernization-plan.md#workflow-structure` and records that pointer in the Saga handoff. A matching differential child releases the U4 pause for U5; `diagnostic`, `inline-only`, `auth-unavailable`, or mismatched readback restores the pause rather than substituting a generic child.
